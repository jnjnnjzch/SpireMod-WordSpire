...
import WordSpire.helpers.DayBeforeCETPlayGameException;
...
public class CallOfCETEvent extends AbstractImageEvent {
    ...
    public static boolean checkBookAvailable(BookEnum b) {
        return WordSpireInitializer.userBooks.contains(WordSpireInitializer.allBooks.get(b));
    }

    public static String getBookOption(BookEnum b) {
        StringBuilder sb = new StringBuilder();
        sb.append(eventStrings.OPTIONS[4]);
        BookConfig config = WordSpireInitializer.allBooks.get(b);
        if (config == null) {
